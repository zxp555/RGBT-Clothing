...
from pytorch3d.renderer.blending import _get_background_color
...
def get_points(infos, dt=0, dr=0, bs=1, wrap=True, random=True, pp_ratio=0.1):
    ...
    centers = torch.tensor(infos.centers).view(-1, 1, 2).expand(-1, bs, -1)
    Rs = torch.tensor(infos.Rs).view(-1, 1, 1).expand(-1, bs, -1)
    ...
    param_wrap = [torch.cartesian_prod(tf, rf)[None, :].repeat(bs, 1, 1) for tf, rf in zip(thetas_wrap, radius_wrap)]
    ...
    thetas = [torch.stack([p[..., 0].cos(), p[..., 0].sin()], -1) for p in params]
...
def softmax_rgb_blend_adasafe(
    ...
    """
    Drop-in replacement for pytorch3d's softmax_rgb_blend.

    On torch 1.12 + CUDA 11.3, torch.prod(...) on Ada GPUs (sm_89) triggers
    an NVRTC JIT path that emits an unsupported -arch flag. This version avoids
    that code path while preserving the original blend semantics closely.
    """
    N, H, W, K = fragments.pix_to_face.shape
    pixel_colors = torch.ones((N, H, W, 4), dtype=colors.dtype, device=colors.device)
    background_color = _get_background_color(
        blend_params, fragments.pix_to_face.device, dtype=colors.dtype
    )
    ...
    if K == 1:
        alpha = 1.0 - prob_map[..., 0]
    else:
        safe_terms = torch.clamp(1.0 - prob_map, min=eps)
        alpha = torch.exp(torch.log(safe_terms).sum(dim=-1))
...
class AdaSafeSoftPhongShader(SoftPhongShader):
    """
    SoftPhongShader variant that avoids torch.prod in alpha blending.
    """
    ...
    def forward(self, fragments, meshes, **kwargs) -> torch.Tensor:
        ...
        return softmax_rgb_blend_adasafe(
